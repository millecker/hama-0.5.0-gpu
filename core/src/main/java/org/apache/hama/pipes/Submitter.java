...
public class Submitter implements Tool {
	...
	/**
	 * Set the URI for the CPU application's executable. Normally this is a
	 * hdfs: location.
	 * 
	 * @param conf
	 * @param executable
	...
	/**
	 * Set the URI for the GPU application's executable. Normally this is a
	 * hdfs: location.
	 * 
	 * @param conf
	 * @param executable
	...
	/**
	 * Submit a job to the cluster. All of the necessary modifications to the
	 * job to run under pipes are made to the configuration.
	 * 
	 * @param conf
	...
	private static void setupPipesJob(BSPJob job) throws IOException {
		 ...
		 * conf.setReducerClass(PipesReducer.class); if
		 * (!getIsJavaRecordWriter(conf)) {
		 ...
		 * gpubin.contains("#")) {
		 * DistributedCache.createSymlink(job.getConf()); // set default gdb
		 * commands for map and reduce task String defScript =
		...
		if (cpubin != null) {
			try {
				...
				IOException ie = new IOException(
						"Problem parsing execable URI " + cpubin);
				ie.initCause(e);
				throw ie;
			}
		}
		if (gpubin != null) {
			try {
				...
				IOException ie = new IOException(
						"Problem parsing execable URI " + gpubin);
				ie.initCause(e);
				throw ie;
			}
		}
	...
	static class CommandLineParser {
		...
		void addOption(String longName, boolean required, String description,
			...
			Option option = OptionBuilder.withArgName(paramName).hasArgs(1)
					.withDescription(description).isRequired(required)
					.create(longName);
	...
	private static <InterfaceType> Class<? extends InterfaceType> getClass(
		...
		return conf.getClassByName((String) cl.getOptionValue(key)).asSubclass(
				cls);
	}
	...
	public int run(String[] args) throws Exception {
		...
		cli.addOption("program", false, "URI to application executable",
				"class");
		cli.addOption("cpubin", false, "URI to application cpu executable",
				"class");
		cli.addOption("gpubin", false, "URI to application gpu executable",
				"class");
		...
		try {
			...
			LOG.info("DEBUG: NormalArguments: "
					+ Arrays.toString(results.getArgs()));
			...
			if (results.hasOption("program")) {
				setExecutable(job.getConf(),
						(String) results.getOptionValue("program"));
			}
