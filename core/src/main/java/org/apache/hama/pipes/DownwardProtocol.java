...
interface DownwardProtocol<K extends WritableComparable, V extends Writable> {
   ...
   * @throws IOException
   */
  void setBSPJob(BSPJob job) throws IOException;
  ...
  void runBsp(InputSplit split, int numReduces, 
		                boolean pipedInput) throws IOException;
  
  /**
   * For maps with pipedInput, the key/value pairs are sent via this message.
   * @param key The record's key
   * @param value The record's value
   * @throws IOException
   */
  //  void mapItem(K key, V value) throws IOException;
  void updateKeyValue(K key, V value) throws IOException;
