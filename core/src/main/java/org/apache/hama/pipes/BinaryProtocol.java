...
class BinaryProtocol<K1 extends Writable, V1 extends Writable, K2 extends Writable, V2 extends Writable>
	...
	private static enum MessageType {
		...
		RUN_SETUP(3), RUN_BSP(4), RUN_CLEANUP(5), READ_KEYVALUE(6), WRITE_KEYVALUE(
				7), GET_MSG(8), GET_MSG_COUNT(9), SEND_MSG(10), SYNC(11), GET_ALL_PEERNAME(
				12), GET_PEERNAME(13), REOPEN_INPUT(14),

		CLOSE(47), ABORT(48), DONE(49), REGISTER_COUNTER(50), INCREMENT_COUNTER(
				51);
	...
	private class UplinkReaderThread extends Thread {
		...
		public UplinkReaderThread(BSPPeer<K1, V1, K2, V2, BytesWritable> peer,
				InputStream stream, K2 key, V2 value) throws IOException {
					...
					if (cmd == MessageType.WRITE_KEYVALUE.code) { // INCOMING
						...
						if (pair != null) {
							WritableUtils.writeVInt(stream,
									MessageType.READ_KEYVALUE.code);
						...
						/* TODO */
						/*
						 * Is not used in HAMA -> Hadoop Pipes - maybe for
						 * performance, skip transferring group and name each
						 * INCREMENT
						 */
	...
	public BinaryProtocol(BSPPeer<K1, V1, K2, V2, BytesWritable> peer,
			Socket sock, K2 key, V2 value) throws IOException {
	 ...
	 */
	public void close(boolean pipedInput, boolean pipedOutput)
